...
const form = useForm({
  site_id: props.location.site_id.toString(),
  building_id: props.location.building_id?.toString() || '',
...
function submit() {
  form.put(locations.update({ location: props.location.id }).url)
}
</script>

<template>
  <Head :title="`Edit Location: ${location.code}`" />
  <AppLayout>
    <div class="container mx-auto py-6 space-y-6">
      ...
      <form @submit.prevent="submit" class="space-y-6">
        ...
        <div class="flex justify-end gap-2">
          ...
          <Button type="submit" :disabled="form.processing">{{ form.processing ? 'Updating...' : 'Update Location' }}</Button>
        </div>
      </form>
    </div>
  </AppLayout>
