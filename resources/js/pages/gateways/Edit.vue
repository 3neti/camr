...
const form = useForm({
  ...
  connection_type: props.gateway.connection_type || '',
  ...
  software_version: props.gateway.software_version || '',
})
...
<template>
  <Head :title="`Edit Gateway: ${gateway.serial_number}`" />

  <AppLayout>
    <div class="container mx-auto py-6 space-y-6">
      <div class="flex items-center gap-4">
        <Link :href="gateways.show({ gateway: gateway.id }).url">
        ...
        <div>
          ...
          <p class="text-muted-foreground">{{ gateway.serial_number }}</p>
        </div>
      </div>
