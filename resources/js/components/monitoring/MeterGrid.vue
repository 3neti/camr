...
import * as meterActions from '@/actions/App/Http/Controllers/MeterController'
...
<template>
  <div>
    <div v-if="loading" class="grid gap-3 sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-2">
      <div v-for="i in 4" :key="i" class="h-28 rounded-lg border bg-muted/30 animate-pulse" />
    </div>

    <div v-else class="grid gap-3 sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-2">
      <Link :href="meterActions.show({ meter: m.meter_id }).url" v-for="m in props.meters" :key="m.meter_id" class="group">
        <Card
          class="transition-colors border-2 hover:shadow-lg h-full group-hover:border-primary/50"
          :class="[
